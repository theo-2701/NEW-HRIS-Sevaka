...
- [x] **Assets** — Asset List, Assigned Assets, Asset Category, Disposal + halaman Asset Detail tanpa baris menu (FSD-COMPANY 0.32 §7–§9 · UIC 0.22 §3.2): registrasi dua bentuk OWNED/LEASED dengan Mandatory-by-Value per cabang; branch/kategori/foto kosong tetap 201 tetapi NOT_AVAILABLE (blocker) sampai lengkap, foto menyusul lewat pintu relay; berkas kontrak sewa sengaja tidak diminta saat registrasi (asimetri §7.3); INCOMPLETE ≠ NOT_AVAILABLE; Return tiga hasil (dua terminal, EMPLOYEE_NEGLIGENCE ditandai klaim); Transfer = dua event RECEIVE lalu GIVING; maintenance SCHEDULED menggeser jadwal, UNSCHEDULED tidak; Disposal hanya dari AVAILABLE, penerima karyawan vs pihak luar dua sub-form, AUCTION satu-satunya yang bisa kembali AVAILABLE. `purchase_date` diperlakukan kolom asli (UIC 0.20 RESOLVED) walau FSD 0.32 masih memasang banner GAP.

// ============================================================
// SEVAKA — Performance Management product shell (mirrors recruitment-shell.js)
...
(function () {
  ...
  var CURFILE = (location.pathname.split('/').pop() || 'performance-cycles.html').toLowerCase();
  ...
  // Performance nav — the 6 menus of FSD-001-PERFORMANCE §1–§6 (TSD §4.1–§4.7).
  var NAV = [
    { key: 'cycles',     label: 'Cycles & Settings',  icon: 'calendar-range',  route: 'performance-cycles.html' },
    { key: 'kpi',        label: 'KPI Master & Weight', icon: 'list-checks',  route: 'performance-kpi-items.html' },
    { key: 'sheets',     label: 'Review Sheets',      icon: 'clipboard-list', route: 'performance-sheets.html' },
    { key: 'approvals',  label: 'Score Approvals',    icon: 'check-check',   route: 'performance-approvals.html' },
    { key: 'objections', label: 'Objections',         icon: 'message-square-warning', route: 'performance-objections.html' },
    { key: 'reports',    label: 'Monitor & Reports',  icon: 'bar-chart-3',   route: 'performance-reports.html' }
  ];
  var SETTINGS = { key: 'settings', label: 'Settings', icon: 'settings' };

  var NOTIFS = [
    { cat: 'approval', icon: 'check-check', iconClass: 'notif-icon--mpp', source: 'SEVAKA Performance', time: '10:05', unread: true, tag: 'Approval', tagClass: 'tag-mpp', msg: 'Review sheet <b>RS-0002</b> (Budi Santoso) is waiting for your decision — cycle 1 / round 1.' },
    { cat: 'objection', icon: 'message-square-warning', iconClass: 'notif-icon--timeoff', source: 'SEVAKA Performance', time: '09:12', unread: true, tag: 'Objection', tagClass: 'tag-timeoff', msg: 'Objection <b>OBJ-0001</b> from Budi Santoso is unanswered.' },
    { cat: 'period', icon: 'calendar-range', iconClass: 'notif-icon--payroll', source: 'SEVAKA Performance', time: 'Yesterday', unread: false, tag: 'Period', tagClass: 'tag-payroll', msg: 'Period <b>Semester 1 2026</b> entered the SIGNING phase on 15 Mar 2026.' }
  ];
    ...
    return (
      ...
      '<div class="product-picker" id="productPicker">' +
        '<button class="product-picker__btn" type="button" id="productPickerBtn" aria-haspopup="listbox">' +
          '<span class="product-picker__now" id="productNow">Performance Management</span>' +
        ...
        '<div class="product-menu" role="listbox">' +
          ...
          PRODUCTS.map(function (p) { return tProductOpt(p, p.id === 'Performance'); }).join('') +
        '</div>' +
      '</div>' +
  ...
  window.PerformanceShell = { render: render, NAV: NAV };
  render();
})();
